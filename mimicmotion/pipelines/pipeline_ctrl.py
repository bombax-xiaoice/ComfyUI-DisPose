...
class Ctrl_Pipeline(DiffusionPipeline):
    ...
    def _encode_image(
        ...
        image_embeddings = self.image_encoder(image).image_embeds.to(device=device)
    ...
    def _encode_pose_image(
        ...
        # Get latents_pose
        pose_latents = self.pose_net(pose_image.to(dtype=next(self.pose_net.parameters()).dtype)).to(device=device)
    ...
    def _encode_vae_image(
        ...
        image_latents = self.vae.encode(image).latent_dist.mode().to(device=device)
    ...
    def __call__(
        ...
        self.image_encoder.to(device) if self.image_encoder.device.type!="meta" else None
        image_embeddings = self._encode_image(image, device, num_videos_per_prompt, self.do_classifier_free_guidance)
        self.image_encoder.cpu() if self.image_encoder.device.type!="meta" else None
        ...
        self.vae.to(device) if self.vae.device.type!="meta" else None
        ...
        self.vae.cpu() if self.vae.device.type!="meta" else None
        ...
        pose_latents = self._encode_pose_image(
            image_pose, do_classifier_free_guidance=self.do_classifier_free_guidance, device=device
        )
        ...
        ref_point_emb = torch.stack(ref_point_emb)
        ...
        self.pose_net.to(device=device) if self.pose_net.device.type!="meta" else None
        self.unet.to(device=device) if self.unet.device.type!="meta" else None
        self.controlnet.to(device=device) if self.controlnet.device.type!="meta" else None
            ...
            for i, t in enumerate(timesteps):
                ...
                for idx in indices:
                    ...
                    down_block_res_samples, mid_block_res_sample, _ = self.controlnet(
                        ...
                        pose_latents=pose_latents[:, idx].flatten(0, 1).to(dtype = self.controlnet.dtype),
                        # pose_image=ctrl_image_pose[:, idx],
                        point_list = point_input,
                        dift_feat = dift_feats[1],
                        ref_point_emb = ref_point_emb if self.controlnet.device.type!='meta' else ref_point_emb.to(device=self._execution_device),
        ...
        self.controlnet.cpu() if self.controlnet.device.type!="meta" else None
        self.unet.cpu() if self.unet.device.type!="meta" else None

        if not output_type == "latent":
            self.vae.decoder.to(device) if self.vae.device.type!="meta" else None

...
class UNetSpatioTemporalConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        if pose_latents is not None:
            sample = sample + (pose_latents if sample.device==pose_latents.device else pose_latents.to(device=sample.device))
        ...
        for downsample_block in self.down_blocks:
            ...
            if down_block_additional_residuals is not None:
                for down_block_res_sample, down_block_additional_residual in zip(
                    ...
                    down_block_res_sample = down_block_res_sample + (down_block_additional_residual if down_block_res_sample.device == down_block_additional_residual.device else down_block_additional_residual.to(device=down_block_res_sample.device))
        ...
        if mid_block_additional_residual is not None:
            sample = sample + (mid_block_additional_residual if sample.device==mid_block_additional_residual.device else mid_block_additional_residual.to(device=sample.device))
